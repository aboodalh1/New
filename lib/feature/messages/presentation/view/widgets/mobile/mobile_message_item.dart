              ...
              children: [
                Text(
                  ...
                  messagesCubit.allMessagesModel.data[index].type=='register'?"Registeration":"Different read",style: TextStyle(
                    color: kSecondaryColor,fontWeight: FontWeight.w500,fontSize: 8.sp
                ),),
                SizedBox(
                    width: 200.w,
                    child: Text(
                      messagesCubit.allMessagesModel.data[index].message,style: TextStyle(fontSize: 8.sp),)),
                Text(messagesCubit.allMessagesModel.data[index].sender??'',style: TextStyle(fontSize: 8.sp),),
              ],
            ),
            const Spacer(),
            Text(messagesCubit.allMessagesModel.data[index].createdAt,style: TextStyle(fontSize: 7.sp),),
          ],
        ),
      );
...
class MobileUnverifiedItem extends StatefulWidget {
  ...
  @override
  State<MobileUnverifiedItem> createState() => _MobileUnverifiedItemState();
}

class _MobileUnverifiedItemState extends State<MobileUnverifiedItem> {
            ...
            Column(
              ...
              children: [
                Text(widget.messagesCubit.allUnverifiedModel.data[widget.index].role,style: TextStyle(
                    color: kPrimaryColor,fontWeight: FontWeight.w500,fontSize: 8.sp
                ),),
                Text(widget.messagesCubit.allUnverifiedModel.data[widget.index].name,style: TextStyle(fontSize:8.sp),),
                Text(widget.messagesCubit.allUnverifiedModel.data[widget.index].phone,style: TextStyle(fontSize:8.sp),),
              ],
            ),
            const Spacer(),
